...
import { deleteListUserbyID,changestatusbyListuser,getUserID } from "../../services/adminServices";
import { getClasses,changestatusbyListclass,deleteListClassbyID } from "../../services/classServices"; 
...
function ManageUser() {
  const columns = [
    {
      ...
      cell: (row) => (
        <>
          <Tooltip text = {userinfor(row)}><div>{row.className}</div></Tooltip>
        </>
      ),
      sortable: true,
    ...
    },
    {
      ...
      cell: ((row) => {
        if (row.status === "Active") {
            return (
            <div>
              <button className="w-[80px] mr-[20px] border-solid  border-[1px] p-[5px] pl-[10px] pr-[10px] bg-[#FF0000]" /*onClick={()=>handleBlockUser(row._id,row.username,row.status)}*/ onClick={()=>openModalstatuschange(addUserToListclass(row._id,row.className,row.status))}>Inactive</button>
              <button className="" /*onClick={()=>handleDeleteUser(row._id,row.username)}*/ onClick={()=>openModalDel(addUserToListclass(row._id,row.className,row.status))}><DeleteIcon/></button>
            </div>);
        } else {
          return (
            <div>
              <button className="w-[80px] mr-[20px] border-solid  border-[1px] p-[5px] pl-[10px] pr-[10px] bg-[#00FF41]" /*onClick={()=>handleBlockUser(row._id,row.username,row.status)}*/ onClick={()=>openModalstatuschange(addUserToListclass(row._id,row.className,row.status))}>Active</button>
              <button className="" /*onClick={()=>handleDeleteUser(row._id,row.username)}*/onClick={()=>openModalDel(addUserToListclass(row._id,row.className,row.status))}><DeleteIcon/></button>
            </div>);
        } 
      })
  ...
  const fetchteacherData = async (teacherID) => {
    try {
      ...
      const teacher = Data.map(user => ({ ID: user._id, fullname: user.fullname }));
  ...
  useEffect(() => {
    ...
    const resultFullname = Object.values(classlist).filter(item => {
      ...
      const teacherInfo = teacherList.find(teacher => teacher.ID === teacherId);
      return teacherInfo && teacherInfo.fullname.toLowerCase().includes(search.toLowerCase());
    });
  
    // Kết hợp kết quả từ cả ba mảng và loại bỏ các phần tử trùng lặp
    const result = Array.from(new Set(resultClassName.concat(resultTitle, resultFullname)));
    setFilter(result);
  }, [search]);
  ...
  const handleDeleteClass = async  (classes) => {
    ...
    try {
      const response = await deleteListClassbyID(ids);
  ...
  const handleRowSelected = React.useCallback(state => {
		setSelectedRows(state.selectedRows.map(s => ({ id: s._id, className: s.className, status: s.status})));
	}, []);
